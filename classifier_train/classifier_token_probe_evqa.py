    ...
    for batch in pbar:
        ...
        if 'original' in model_type:
            if 'wo_rir' in model_type:
                # 只使用第一个图像的表征
                x = original_image_hidden # (batch_size, 1, 4096)
        ...
        if 'wo_rir' in model_type:
            ...
            # print("text_hidden shape: ", text_hidden.shape)
        else:
            # 使用两种文本表征
        ...
        # x = text_hidden.squeeze(1).squeeze(1)
        # print("x shape: ", x.shape)
...
def save_model(model, model_name, epoch):
    save_dir = f'models/token_probe_infoseek_i3_new/{model_name}'
    ...
    for num_classes in [4]:
        ...
        for layer_idx in layer_indices:
            ...
            dataset = TokenProbeDataset(
                ...
                json_file='data_0921/infoseek_i3/infoseek_train_i3_rir/Qwen25_72B_Instruct_awq_vllm_l20_judged_logs_infoseek_train_i3_rir.json',
                image_hidden_state_dir='hidden_state_i3_train/hidden_infoseek/hidden_state',
                original_image_hidden_state_dir='hidden_state_i3_train/hidden_infoseek/original_hidden_state',
                hidden_state_dir='hidden_state_i3_train/hidden_infoseek/last_hidden_state',
                hidden_state_rir_dir='hidden_state_i3_train/hidden_infoseek/last_hidden_state_rir',
                layer_idx=layer_idx
            )
            ...
            model_configs = [
                ...
                # (f'qwen_dp3_{task_prefix}_layer_{layer_idx}_original_image_wo_rir', 7168, num_classes),
                # (f'qwen_dp3_{task_prefix}_layer_{layer_idx}_image_wo_rir', 3584, num_classes),
                # 使用所有表征的模型 (输入维度 8192 + 8192 = 16384)
                # (f'qwen_dp3_{task_prefix}_layer_{layer_idx}_original_image_rir', 16384, num_classes),
                # (f'qwen_dp3_{task_prefix}_layer_{layer_idx}_image_rir', 7168, num_classes)

                (f'qwen_dp3_{task_prefix}_layer_{layer_idx}_image_wo_rir', 8192, num_classes),
                # 使用所有表征的模型 (输入维度 8192 + 8192 = 16384)
                # (f'qwen_dp3_{task_prefix}_layer_{layer_idx}_original_image_rir', 16384, num_classes),
                # (f'qwen_dp3_{task_prefix}_layer_{layer_idx}_image_rir', 14336, num_classes)
            ...
            for model_name, input_dim, num_classes in model_configs:
                ...
                if num_classes == 2:
                    ...
                    criterion = nn.CrossEntropyLoss(weight=torch.tensor([1.0, 20.0, 10.0, 50.0]).to(device))
