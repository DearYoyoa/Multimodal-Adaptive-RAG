...
def query_with_image(
    ...
    if use_screenshot:
        ...
        # if not os.path.exists(f'hidden_evqa/last_hidden_state_rir'):
        #     os.makedirs(f'hidden_evqa/last_hidden_state_rir')
        # np.save(f'hidden_evqa/last_hidden_state_rir/{image_path.split("/")[-1].split(".")[0]}.npy', hidden_state_with_screenshot)
    else:
        with torch.no_grad():
            model_output = model(
                **inputs,
        ...
        # if not os.path.exists(f'hidden_evqa/last_hidden_state'):
        #     os.makedirs(f'hidden_evqa/last_hidden_state')
        # np.save(f'hidden_evqa/last_hidden_state/{image_path.split("/")[-1].split(".")[0]}.npy', hidden_state_without_screenshot)

    if extract_vision_feature:
        vision_features = extract_vision_features(model_output, inputs, model)
        print("vision_shape:",vision_features.shape)
        ...
        if not os.path.exists(f'data_image_token_i2_train/data_image_token_okvqa_max/hidden_state/'):
            os.makedirs(f'data_image_token_i2_train/data_image_token_okvqa_max/hidden_state/')
        np.save(f'data_image_token_i2_train/data_image_token_okvqa_max/hidden_state/{image_path.split("/")[-1].split(".")[0]}.npy', vision_features)
        if not os.path.exists(f'data_image_token_i2_train/data_image_token_okvqa_max/original_hidden_state/'):
            os.makedirs(f'data_image_token_i2_train/data_image_token_okvqa_max/original_hidden_state/')
        np.save(f'data_image_token_i2_train/data_image_token_okvqa_max/original_hidden_state/{image_path.split("/")[-1].split(".")[0]}.npy', original_vision_features.cpu().numpy())
