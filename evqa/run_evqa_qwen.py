...
def query_with_image(
    ...
    if not os.path.exists(f'hidden_state_qwen_train/hidden_evqa_text/hidden_state_rir/'):
        os.makedirs(f'hidden_state_qwen_train/hidden_evqa_text/hidden_state_rir/')
    np.save(f'hidden_state_qwen_train/hidden_evqa_text/hidden_state_rir/hidden_states_rir_{image_path.split("/")[-1].split(".")[0]}.npy', last_hidden_state)
    ...
    if extract_vision_feature:
        ...
        if not os.path.exists(f'hidden_state_qwen_train/hidden_evqa_image_mean/hidden_state/'):
            os.makedirs(f'hidden_state_qwen_train/hidden_evqa_image_mean/hidden_state/')
        np.save(f'hidden_state_qwen_train/hidden_evqa_image_mean/hidden_state/{image_path.split("/")[-1].split(".")[0]}.npy', vision_features)
        if not os.path.exists(f'hidden_state_qwen_train/hidden_evqa_image_mean/original_hidden_state/'):
            os.makedirs(f'hidden_state_qwen_train/hidden_evqa_image_mean/original_hidden_state/')
        np.save(f'hidden_state_qwen_train/hidden_evqa_image_mean/original_hidden_state/{image_path.split("/")[-1].split(".")[0]}.npy', original_vision_features.cpu().numpy())    # 处理生成的文本
...
def main(args):
    ...
    # ).to(DEVICE)
    model = Qwen2VLForConditionalGeneration.from_pretrained("/ossfs/workspace/Qwen__Qwen2-VL-7B-Instruct", ignore_mismatched_sizes=True).to(DEVICE)
