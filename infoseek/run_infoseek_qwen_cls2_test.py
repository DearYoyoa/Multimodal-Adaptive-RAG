...
def query_with_image(
    ...
    # if last_hidden_without_screenshot is not None:
    #     print("last_hidden_without_screenshot_0: ", last_hidden_without_screenshot.shape)
    # if last_hidden_with_screenshot is not None:
    #     print("hidden_state_with_screenshot_0: ", last_hidden_with_screenshot.shape)
    # if image_hidden_state is not None:
    #     print("image_hidden_state_0: ", image_hidden_state.shape)
    # if original_vision_features is not None:
    #     print("original_vision_features_0: ", original_vision_features.shape)
    ...
    with torch.no_grad():
        if use_rir:
            ...
            if 'original' in classifier_path:
                x = original_vision_features.to(DEVICE)
            else:
            ...
            x = torch.cat((x, hidden_state_without_screenshot.squeeze(0).squeeze(0)), dim=0)
            # print("x.shape:", x.shape)
        # x = hidden_state_with_screenshot.squeeze(1)
...
def main(args):
    ...
    # input_dim = 14336 if use_rir else 7168
    input_dim = 14336 if use_rir else 7168
    ...
    with open('local_data/infoseek_qwen_rir/infoseek_qwen_test_rir/Qwen25_72B_Instruct_awq_vllm_l20_judged_logs_infoseek_qwen_test_rir.json', 'r') as f:
        logs_test_rir = json.load(f)
    
    with open('local_data/infoseek_qwen/infoseek_qwen_test/Qwen25_72B_Instruct_awq_vllm_l20_judged_logs_infoseek_qwen_test.json', 'r') as f:
    ...
    for idx, sample in tqdm(enumerate(samples[args.idx_offset:]), total=len(samples[args.idx_offset:])):
        ...
        last_hidden_rir = np.load(f"hidden_state_qwen_test/hidden_infoseek_text/hidden_state_rir/hidden_states_rir_{sample['image_id'].split('.')[0]}.npy")
        last_hidden = np.load(f"hidden_state_qwen_test/hidden_infoseek_text/hidden_state/hidden_states_{sample['image_id'].split('.')[0]}.npy")
        image_hidden = np.load(f"hidden_state_qwen_test/hidden_infoseek_image_mean/hidden_state/{sample['image_id'].split('.')[0]}.npy")
        original_hidden = np.load(f"hidden_state_qwen_test/hidden_infoseek_image_mean/original_hidden_state/{sample['image_id'].split('.')[0]}.npy")
