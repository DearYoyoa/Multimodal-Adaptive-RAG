...
def query_with_image(
    ...
    if extract_vision_feature:
        ...
        if not os.path.exists(f'hidden_state_i3_train/hidden_infoseek/hidden_state/'):
            os.makedirs(f'hidden_state_i3_train/hidden_infoseek/hidden_state/')
        np.save(f'hidden_state_i3_train/hidden_infoseek/hidden_state/{image_path.split("/")[-1].split(".")[0]}.npy', vision_features)
        if not os.path.exists(f'hidden_state_i3_train/hidden_infoseek/original_hidden_state/'):
...
def main(args):
    ...
    for idx, sample in tqdm(enumerate(samples[args.idx_offset:]), total=len(samples[args.idx_offset:])):
        ...
        image_path = f"data_0921/image/{sample['image_id']}.JPEG"
        screenshot_path = f"data_0921/screenshot/{sample['image_id']}-search_result.png"
