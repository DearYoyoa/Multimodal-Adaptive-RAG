...
def main(args):
    ...
    with open('local_data/infoseek_i3/infoseek_test_i3_rir/Qwen25_72B_Instruct_awq_vllm_l20_judged_logs_infoseek_test_i3_rir.json', 'r') as f:
        logs_test_rir = json.load(f)
    
    with open('local_data/infoseek_i3/infoseek_test_i3/Qwen25_72B_Instruct_awq_vllm_l20_judged_logs_infoseek_test_i3.json', 'r') as f:
    ...
    for idx, sample in tqdm(enumerate(samples[args.idx_offset:]), total=len(samples[args.idx_offset:])):
        ...
        last_hidden_rir = np.load(f"hidden_state_i3_test/hidden_infoseek/last_hidden_state_rir/{sample['image_id'].split('.')[0]}.npy")
        last_hidden = np.load(f"hidden_state_i3_test/hidden_infoseek/last_hidden_state/{sample['image_id'].split('.')[0]}.npy")
        image_hidden = np.load(f"hidden_state_i3_test/hidden_infoseek/hidden_state/{sample['image_id'].split('.')[0]}.npy")
        original_hidden = np.load(f"hidden_state_i3_test/hidden_infoseek/original_hidden_state/{sample['image_id'].split('.')[0]}.npy")
