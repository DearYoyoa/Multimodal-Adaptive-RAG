...
def main(args):
    ...
    for idx, sample in tqdm(enumerate(samples[args.idx_offset:]), total=len(samples[args.idx_offset:])):
        ...
        # image_hidden = np.load(f"hidden_state_i2_test/hidden_okvqa_rir_max/hidden_state/{sample['image_id']}.npy")
        # original_hidden = np.load(f"hidden_state_i2_test/hidden_okvqa_rir_max/original_hidden_state/{sample['image_id']}.npy")
        
        image_path = f"data_okvqa/okvqa_image/{sample['image_id']}.jpg"
        screenshot_path = f"data_okvqa/screenshot/{sample['image_id']}-search_result.png"
        # last_hidden = np.load(f"hidden_state_i2_test/hidden_okvqa/last_hidden_state/{sample['image_id']}.npy")
        last_hidden = np.load(f"hidden_state_i2_test/hidden_okvqa/exact_answer_features_mlp/{token_type}/{sample['image_id']}.npy")
        image_hidden = np.load(f"hidden_state_i2_test/hidden_okvqa_image_all/hidden_state/{sample['image_id']}.npy")
        original_hidden = np.load(f"hidden_state_i2_test/hidden_okvqa_image_all/original_hidden_state/{sample['image_id']}.npy")
