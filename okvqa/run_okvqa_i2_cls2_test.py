...
def query_with_image(
    ...
    with torch.no_grad():
        if use_rir:
            ...
            # x = torch.cat((x, hidden_state_without_screenshot.squeeze(1)), dim=1)
            x = hidden_state_without_screenshot.squeeze(1)
            # print("x.shape:", x.shape)
            # print("x.shape:", x.shape)
...
def main(args):
    ...
    with open('data_okvqa/okvqa/okvqa_i2_test_rir/Qwen25_72B_Instruct_awq_vllm_l20_judged_logs_okvqa_i2_test_rir.json', 'r') as f:
    ...
    for idx, sample in tqdm(enumerate(samples[args.idx_offset:]), total=len(samples[args.idx_offset:])):
        ...
        last_hidden_rir = np.load(f"hidden_state_i2_test/hidden_okvqa/last_hidden_state_rir/{sample['image_id']}.npy")
        last_hidden = np.load(f"hidden_state_i2_test/hidden_okvqa/last_hidden_state/{sample['image_id']}.npy")
        # last_hidden_rir = np.load(f"hidden_state_i2_test/hidden_okvqa/exact_answer_features_mlp_rir/{args.token_type}/{sample['image_id']}.npy")
        # last_hidden = np.load(f"hidden_state_i2_test/hidden_okvqa/exact_answer_features_mlp/{args.token_type}/{sample['image_id']}.npy")
