...
def query_with_image(
    ...
    if extract_vision_feature:
        ...
        original_vision_features = model_output.image_hidden_states.reshape(2, -1, 4096)
        if not os.path.exists(f'hidden_state_i2_test/hidden_okvqa_image_all/hidden_state/'):
            os.makedirs(f'hidden_state_i2_test/hidden_okvqa_image_all/hidden_state/')
        np.save(f'hidden_state_i2_test/hidden_okvqa_image_all/hidden_state/{image_path.split("/")[-1].split(".")[0]}.npy', vision_features)
        if not os.path.exists(f'hidden_state_i2_test/hidden_okvqa_image_all/original_hidden_state/'):
            os.makedirs(f'hidden_state_i2_test/hidden_okvqa_image_all/original_hidden_state/')
        np.save(f'hidden_state_i2_test/hidden_okvqa_image_all/original_hidden_state/{image_path.split("/")[-1].split(".")[0]}.npy', original_vision_features.cpu().numpy())    # 处理生成的文本
    generated_ids = 0
