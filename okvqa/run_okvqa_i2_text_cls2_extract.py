...
def extract_vision_features(generated_output, inputs, model):
    ...
    for layer_idx, layer_hidden_states in enumerate(all_hidden_states):
        ...
        for i in range(len(image_boundaries) - 1):
            ...
            image_feature = image_patches.max(dim=1)[0]  # [batch_size, hidden_dim]
...
def query_with_image(
    ...
    if extract_vision_feature:
        ...
        original_vision_features = model_output.image_hidden_states.reshape(2, -1, 4096).max(dim=1)[0]
        ...
        if not os.path.exists(f'data_image_token_i2_train/data_image_token_okvqa_max/origial_hidden_state/'):
            os.makedirs(f'data_image_token_i2_train/data_image_token_okvqa_max/origial_hidden_state/')
        np.save(f'data_image_token_i2_train/data_image_token_okvqa_max/origial_hidden_state/{image_path.split("/")[-1].split(".")[0]}.npy', original_vision_features.cpu().numpy())
