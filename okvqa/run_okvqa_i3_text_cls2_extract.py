...
def extract_vision_features(generated_output, inputs, model):
    ...
    all_hidden_states = generated_output.hidden_states  
    ...
    for layer_idx, layer_hidden_states in enumerate(all_hidden_states):
        ...
        for i in range(len(image_boundaries) - 1):
            ...
            #     model.vision_attention(query=image_patches.mean(1), 
            #     key=image_patches,
            #     value=image_patches), dim=-1)
            # image_feature = torch.einsum('bnd,bn->bd', image_patches, attention_weights)
...
def query_with_image(
    ...
    if use_screenshot:
        ...
        if not os.path.exists(f'hidden_state_i3_test/hidden_okvqa_2/last_hidden_state_rir'):
            os.makedirs(f'hidden_state_i3_test/hidden_okvqa_2/last_hidden_state_rir')
        np.save(f'hidden_state_i3_test/hidden_okvqa_2/last_hidden_state_rir/{image_path.split("/")[-1].split(".")[0]}.npy', hidden_state_with_screenshot)
    else:
        with torch.no_grad():
            model_output = model(
                **inputs,
        ...
        # if not os.path.exists(f'hidden_state_i3_test/hidden_okvqa_2/last_hidden_state'):
        #     os.makedirs(f'hidden_state_i3_test/hidden_okvqa_2/last_hidden_state')
        # np.save(f'hidden_state_i3_test/hidden_okvqa_2/last_hidden_state/{image_path.split("/")[-1].split(".")[0]}.npy', hidden_state_without_screenshot)
    ...
    if extract_vision_feature:
        ...
        if not os.path.exists(f'hidden_state_i3_test/hidden_okvqa_2/hidden_state/'):
            os.makedirs(f'hidden_state_i3_test/hidden_okvqa_2/hidden_state/')
        np.save(f'hidden_state_i3_test/hidden_okvqa_2/hidden_state/{image_path.split("/")[-1].split(".")[0]}.npy', vision_features)
        if not os.path.exists(f'hidden_state_i3_test/hidden_okvqa_2/original_hidden_state/'):
            os.makedirs(f'hidden_state_i3_test/hidden_okvqa_2/original_hidden_state/')
        np.save(f'hidden_state_i3_test/hidden_okvqa_2/original_hidden_state/{image_path.split("/")[-1].split(".")[0]}.npy', original_vision_features.cpu().numpy())
