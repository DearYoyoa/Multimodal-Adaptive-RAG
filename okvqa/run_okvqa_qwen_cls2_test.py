...
def main(args):
    ...
    input_dim = 7168 if use_rir else 7168
    ...
    with open('data_okvqa/okvqa_qwen_rir/okvqa_qwen_test_rir/Qwen25_72B_Instruct_awq_vllm_l20_judged_logs_okvqa_qwen_test_rir.json', 'r') as f:
        logs_test_rir = json.load(f)
    
    with open('data_okvqa/okvqa_qwen/okvqa_qwen_test/Qwen25_72B_Instruct_awq_vllm_l20_judged_logs_okvqa_qwen_test.json', 'r') as f:
    ...
    for idx, sample in tqdm(enumerate(samples[args.idx_offset:]), total=len(samples[args.idx_offset:])):
        ...
        last_hidden_rir = np.load(f"hidden_state_qwen_test/hidden_okvqa_text/hidden_state_rir/hidden_states_rir_{sample['image_id']}.npy")
        last_hidden = np.load(f"hidden_state_qwen_test/hidden_okvqa_text/hidden_state/hidden_states_{sample['image_id']}.npy")
        image_hidden = np.load(f"hidden_state_qwen_test/hidden_okvqa_image_mean/hidden_state/{sample['image_id']}.npy")
        original_hidden = np.load(f"hidden_state_qwen_test/hidden_okvqa_image_mean/original_hidden_state/{sample['image_id']}.npy")
